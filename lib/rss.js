...
/**
 * Custom Feed class to include custom XML elements
 */
class CustomFeed extends Feed {
  rss2() {
    const baseRss = super.rss2();
    const customContent = `
      <follow_challenge>
        <feedId>60613242413328384</feedId>
        <userId>55922905684581376</userId>
      </follow_challenge>
    `;
    return baseRss.replace('</channel>', `${customContent}</channel>`);
  }
}
...
export async function generateRss(props) {
  const { NOTION_CONFIG, siteInfo, posts } = props
